...
public class B2BProductDetailsActivity extends BaseActivity {
    ...
    private void initViews() {
        ...
        quantityView.setOnQuantityChangeListener(new QuantityView.OnQuantityChangeListener() {
            ...
            public void onQuantityChanged(int oldQuantity, int newQuantity, boolean programmatically) {
                ...
                if (newQuantity <= product.getSubProductList().get(subProduct).getAvailableQuantityInKgOrLtr()) {
                    Qty = newQuantity;
    ...
    private void setData() {
        ...
        if (product.getSubProductList().get(subProduct).getProductSellingPrice() != null) {
            tvMRP.setText(Html.fromHtml("<strike> ₹ " + product.getSubProductList().get(subProduct).getProductSellingPrice().toString() + "</strike>"));
        } else
            tvMRP.setText(Html.fromHtml("<strike> ₹ " + 0.0 + "</strike>"));
        if (product.getSubProductList().get(subProduct).getProductPricePerGramOrMl() != null) {
            tvSellPrice.setText("₹ " + product.getSubProductList().get(subProduct).getProductPricePerGramOrMl() * 1000);
        } else
            tvSellPrice.setText("₹ " + 0.0);

        tvWeight.setText("1Kg / 1Ltr");
        tvDesc.setText(product.getSubProductList().get(subProduct).getProductDesc());
        if (product.getSubProductList().get(subProduct).getAvailableQuantityInKgOrLtr() != null) {
            tvAvailableStock.setText("Available Stock : " + product.getSubProductList().get(subProduct).getAvailableQuantityInKgOrLtr());
        } else
            tvAvailableStock.setText("Available Stock : " + 0.0);


        if (product.getSubProductList().get(subProduct).getMinQuantityInKgOrLtr() != null && product.getSubProductList().get(subProduct).getAvailableQuantityInKgOrLtr() != null) {
        ...
        btnBuyNow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (Qty != 0 && Qty >= product.getSubProductList().get(subProduct).getMinQuantityInKgOrLtr()) {
                    BulkTotalAmount = product.getSubProductList().get(subProduct).getProductPricePerGramOrMl() * 1000 * Qty;
                    //OrderType = 1;
                    ...
                    showToast("Please Select Product Quantity Minimum " + product.getSubProductList().get(subProduct).getMinQuantityInKgOrLtr());
            }
        });
